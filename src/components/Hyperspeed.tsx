...
const Hyperspeed = ({
  ...
  // Trigger speed up/down based on isTurbo prop
  useEffect(() => {
    if (!appRef.current) return;
    if (isTurbo) {
      if (appRef.current.options.onSpeedUp) appRef.current.options.onSpeedUp();
      appRef.current.fovTarget = appRef.current.options.fovSpeedUp;
      appRef.current.speedUpTarget = appRef.current.options.speedUp;
    } else {
      if (appRef.current.options.onSlowDown) appRef.current.options.onSlowDown();
