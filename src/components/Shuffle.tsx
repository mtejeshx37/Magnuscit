...
// Reverting to user specific import style for consistency with request:
// @ts-ignore
import { SplitText as GSAPSplitTextImport } from 'gsap/SplitText';

// Re-assigning to use in logic
const GSAPSplitText = GSAPSplitTextImport;

try {
    gsap.registerPlugin(ScrollTrigger, GSAPSplitText, useGSAP);
} catch (e) {
    console.warn("GSAP plugins registration failed. Ensure dependencies are installed.");
}
...
const Shuffle: React.FC<ShuffleProps> = ({
    ...
    const ref = useRef<any>(null); // Using any for dynamic tag ref
    ...
    useGSAP(
        () => {
            ...
            const play = () => {
                ...
                const addTween = (targets: gsap.TweenTarget, at: number | string) => {
                ...
                if (animationMode === 'evenodd') {
                    ...
                    const oddTotal = (duration || 0) + Math.max(0, odd.length - 1) * (stagger || 0.03);
                    const evenStart = odd.length ? oddTotal * 0.7 : 0;
                    if (odd.length) addTween(odd, 0);
                    if (even.length) addTween(even, evenStart);
                } else {
                    strips.forEach(strip => {
                        const d = Math.random() * (maxDelay || 0);
                        const vars: gsap.TweenVars = {
                            duration,
                            ease,
                            force3D: true
